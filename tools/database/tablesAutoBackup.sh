...
# TODO
# /usr/bin/mysqldump --opt -ubatsing -pbatsingpw -hlocalhost timepusher > /home/data/test/mysqlBackup/db_`date +%F`.sql
# $BIN_DIR/mysqldump --opt -u$DB_USER -p$DB_PASS -h$DB_HOST $DB_NAME > $BCK_DIR/db_$DATE.sql
$BIN_DIR/mysqldump --opt -u$DB_USER -p$DB_PASS -h$DB_HOST $DB_NAME t_device t_order > $BCK_DIR/tables_$DATE.sql

# Find backup file and delete（save 30 days）
find $BCK_DIR -name "tables_*.sql" -type f -mtime +30 -exec rm {} \; > /dev/null 2>&1
